...
3. 채팅에서 질문에 답변한다.
4. 새 질문은 기본적으로 **직전 `### Qn.` 항목 안에** 이어지는 후속 질문(같은 주제를 파고드는 질문)으로 취급해서, 그 항목 밑에 인용문(`>`)으로 들여쓴 Q/A 쌍으로 추가한다. 사용자가 화제를 바꾸거나 새 주제로 명시적으로 넘어가면 그때 `### Qn+1.`로 새 번호를 만든다 (n은 그 `QnA/` 파일에서 가장 큰 기존 번호 + 1). 플레이스홀더 주석 `<!-- 다시 읽으면서... -->`이 남아있다면 첫 등록 시 지운다. 기존 Q&A 항목은 건드리지 않는다. **`### Qn.` 제목 줄 자체는 들여쓰지 않고, 그 하위 Q/A 쌍만 `>`로 들여써서 가독성을 확보한다** (Qn이 부모, Q/A가 자식이라는 위계를 시각적으로 드러내기 위함).
   ...
   한 Q/A 안에 문단이 여러 개면 문단 사이에도 `>` 만 있는 줄을 넣어 인용 블록을 유지한다. 사용자가 여러 개의 개별 Q&A 항목을 하나의 번호로 묶어달라고 요청하면, 그 항목들을 위 형식으로 합쳐서 하나의 `### Qn.`으로 재구성한다.
